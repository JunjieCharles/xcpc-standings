...
- 单年短写：`25`
- 年份范围：`2021-2025`
- 半年度范围：`25下半年-26上半年` 或 `25H2-26H1`，按比赛日期落入对应半年度筛选。
- 全量：`all`
...
```text
Rankland -> XCPCIO -> PTA -> Archive -> PDF
```
...
```text
Online < Preliminary < Regional < Final
CCPC < ICPC
```

个人 rating 的 XLSX 输出中，同一天多场比赛合并成一个展示列，tag 由当日比赛名拼接；个人 rating 的 CSV 输出不合并同一天的多个比赛，一场比赛一个列。学校 rating 中，同一天比赛不合并展示列，而是一场比赛一个列。
