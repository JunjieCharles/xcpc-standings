...
def get_category(series, idx, name):
    ...
    if series == 'Other':
        if 'warmup' in lb or '热身' in ln or 'dressrehearsal' in lb or 'dress rehearsal' in ln:
            return 'Warmup'
        return 'Regular'
        
    if 'warmup' in lb or '热身' in ln or 'dressrehearsal' in lb or 'dress rehearsal' in ln:
        return 'Warmup'
    if 'girl' in lb or 'girl' in ln or 'lad' in lb or 'lad' in ln or '濂崇敓' in ln:
        return 'Girls'
    ...
    if 'final' in lb or '总决赛' in ln:
        return 'Final'
    if 'provincial' in lb or '省赛' in ln:
        return 'Provincial'
...
def get_name_id(series, idx):
    ...
    idx_cleaned = re.sub(r'[^a-z0-9]', '', idx_cleaned)
    if series != 'Other':
        idx_cleaned = re.sub(r'(icpc|ccpc)', '', idx_cleaned)
    idx_cleaned = re.sub(r'\d{4}', '', idx_cleaned)
    idx_cleaned = re.sub(r'\d+(th|st|nd|rd)', '', idx_cleaned)
    idx_cleaned = re.sub(r'\d+', '', idx_cleaned) # remove any remaining naked ordinals
    for w in ['invitational', 'online', 'onsite', 'warmup', 'camp', 'provincial', 'contest', 'internet', 'girls', 'girl', 'ladies', 'lady', 'women', 'dressrehearsal']:
...
def merge_contests(records):
    ...
    for r in records:
        ...
        name_id = get_name_id(series, cid)
...
def main():
    ...
    all_contests = xcpcio_list + rankland_list + archive_list
    ...
    out_fields = ['series', 'year', 'ordinal', 'date', 'category', 'name', 'xcpcio_id', 'rankland_id', 'archive_id']
    try:
