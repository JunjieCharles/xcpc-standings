...
from src.providers import ArchiveProvider, XCPCIOProvider, RanklandProvider, PTAProvider, PDFProvider
from src.sources.pdf_source import find_pdf_identifier
...
def merge_pdf_roster(base_json: Dict, pdf_json: Dict) -> Tuple[Dict, int, int]:
    base_cs = ContestStandings.from_dict(base_json)
    pdf_cs = ContestStandings.from_dict(pdf_json)

    roster_by_key = {}
    for team in pdf_cs.standings:
        key = (normalize_text(team.school), normalize_text(team.team_name))
        if key[0] and key[1]:
            roster_by_key[key] = team

    matched = 0
    for team in base_cs.standings:
        key = (normalize_text(team.school), normalize_text(team.team_name))
        roster = roster_by_key.pop(key, None)
        if not roster:
            continue

        matched += 1
        if not team.member1 and roster.member1:
            team.member1 = roster.member1
        if not team.member2 and roster.member2:
            team.member2 = roster.member2
        if not team.member3 and roster.member3:
            team.member3 = roster.member3
        if not team.coach and roster.coach:
            team.coach = roster.coach

    return base_cs.to_dict(), matched, len(roster_by_key)
...
def batch_process(year_arg="2025"):
    ...
    for idx, row in target_df.iterrows():
        ...
        pdf_id = find_pdf_identifier(row.to_dict())
        ...
        print(f"\nProcessing: {name} (XCPCIO={xcpcio_id}, Rankland={rankland_id}, Archive={archive_id}, PTA={pta_id}, PDF={pdf_id})")
        ...
        pdf_jsons = [(src_name, src_json) for src_name, src_json in jsons if src_name == "PDF"]
        jsons = [(src_name, src_json) for src_name, src_json in jsons if src_name != "PDF"]

        if not jsons and pdf_jsons:
            jsons = pdf_jsons
            pdf_jsons = []
        ...
        for _pdf_name, pdf_json in pdf_jsons:
            current_merged, matched, unmatched = merge_pdf_roster(current_merged, pdf_json)
            print(f"  Merged PDF roster: matched {matched} teams, unmatched {unmatched} teams")
...
if __name__ == "__main__":
    ...
    if args.batch:
        ...
        with open(f"data/merged/json/{out_n}.json", "w", encoding="utf-8", newline='\n') as f:
