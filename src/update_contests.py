...
def parse_rankland_config(data):
    ...
    def walk(node, series='Other', source_category='', year=None, ordinal=None, context_names=None):
        ...
        if has_children:
            for child in children:
                next_context = context_names + [str(node.get('name', '') or '')]
                walk(child, series, source_category, year, ordinal, next_context)
            return
        ...
        contests.append({
            ...
            'date': date_str,
            'name': name,
            'source_context': ' '.join(value for value in context_names if value),
            'id': path
        })
...
def match_chinese_to_eng(text):
    lookup_text = str(text or '').lower()
    for ch, en in sorted(get_chinese_to_eng().items(), key=lambda item: len(item[0]), reverse=True):
        if ch.lower() in lookup_text:
            return en
    return ''

def get_provincial_name_id(year, category, idx, name, source_context=''):
    if category != 'Provincial' or str(year) not in PROVINCIAL_NAME_YEARS:
        return ''
    lookup_text = ' '.join(str(value or '') for value in [name, source_context, idx])
    return match_chinese_to_eng(lookup_text)

def get_pta_location_name_id(source, category, name):
    if source != 'pta' or category not in ('Invitational', 'Preliminary'):
        return ''
    return match_chinese_to_eng(name)
...
def get_name_id(source, series, idx, name):
    ...
    idx_cleaned = idx_cleaned.replace("网络预选赛1", "online1")
    idx_cleaned = idx_cleaned.replace("网络预选赛2", "online2")
    idx_cleaned = idx_cleaned.replace("网络赛1", "online1")
    idx_cleaned = idx_cleaned.replace("网络赛2", "online2")
    ...
    idx_cleaned = idx_cleaned.replace("onlinequalification", "online")
    ...
    for w in ['总决赛', '预选赛', '预赛', '邀请赛', '省赛', '特训营', '夏令营', '热身赛', '系统测试赛', '程序设计竞赛', '计算机技能竞赛', '高职专场', '女生专场', '混合组', '组', '锦标赛', '第十一届', '第十二届', '第十届', '届', '大学生', '中国']:
        idx_cleaned = idx_cleaned.replace(w, '')
        
    for w in ['invitational', 'preliminary', 'onsite', 'warmup', 'camp', 'provincial', 'contest', 'internet', 'girls', 'girl', 'ladies', 'lady', 'women', 'dressrehearsal']:
        idx_cleaned = idx_cleaned.replace(w, '')
        
    aliases = {
        'northeastern': 'northeast',
    }
    return aliases.get(idx_cleaned.strip(), idx_cleaned.strip())
...
def get_contest_key(year, series, category, name_id, date):
    if category == 'Provincial' and str(year) in PROVINCIAL_NAME_YEARS:
        return (str(year), str(series), str(category), str(name_id), str(date))
    return (str(year), str(series), str(category), str(name_id))
...
def merge_contests(records):
    ...
    for r in records:
        ...
        name_id = get_provincial_name_id(year, sub, cid, name, source_context)
        if not name_id:
            name_id = get_pta_location_name_id(source, sub, name)
        if not name_id:
            name_id = get_name_id(source, series, cid, name)
        name_id = get_default_name_id(sub, name_id)
        
        # Recent provincial contests use date in the merge key because one province can have multiple events in a year.
        key = get_contest_key(year, series, sub, name_id, date)
    ...
    for key, row in merged_dict.items():
        ...
        has_target_provincial_name = row.get('category') == 'Provincial' and str(row.get('year')) in PROVINCIAL_NAME_YEARS
        has_required_short_name = has_target_provincial_name or row.get('category') in ('Invitational', 'Preliminary', 'Vocational')
        if has_only_pta and pta_full_names.get(key) and not has_required_short_name:
