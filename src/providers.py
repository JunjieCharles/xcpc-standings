...
class PDFProvider(JSONCacheProvider):
    source_name = "PDF"

    def __init__(self, identifier: str, contest_name: str):
        super().__init__(identifier, contest_name, "data/raw/cache/pdf", "data/raw/json/pdf")

    def is_valid(self) -> bool:
        return bool(self.identifier) and os.path.exists(os.path.join(self.cache_dir, self.identifier))

    def fetch_raw(self):
        from src.sources.pdf_source import PDFDataSource
        print(f"  [{self.source_name}] Loading local PDF: {self.identifier}")
        return PDFDataSource().fetch_contest_data(self.identifier)

    def parse_standard(self, raw_data):
        from src.sources.pdf_source import PDFStandingsGenerator
        std = PDFStandingsGenerator(raw_data, self.contest_name).generate()
        if "contest_name" not in std or not std["contest_name"]:
            std["contest_name"] = self.contest_name
        return std
